...
use super::theme::*;
use super::*;
use crate::structs::{DynamicRenderMode, GraphicsQuality, OptimizationProfile};
...
impl BoosterApp {
    pub(crate) fn render_boost_tab(&mut self, ui: &mut egui::Ui) {
        ...
        // System Boosts Grid
        self.render_boost_category_grid(
            ui,
    ...
    fn render_boost_master_toggle(&mut self, ui: &mut egui::Ui) {
        ...
        let has_pending_changes = self.boost_changes_pending;
        ...
        card_frame().show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                if is_active {
                    ...
                    let pulse = ((elapsed * std::f32::consts::PI / PULSE_ANIMATION_DURATION).sin()
                        + 1.0)
                        / 2.0;
                    ...
                    ui.painter()
                        .circle_stroke(center, 14.0, egui::Stroke::new(1.5, status_color));
                }
                ...
                ui.vertical(|ui| {
                    ...
                    let desc = if has_pending_changes {
                        "Changes pending - press Apply".to_string()
                    } else if is_active {
                        format!("{} boosts enabled - {} profile", active_count, profile_name)
                    } else {
                        "Select boosts and press Apply".to_string()
                    };
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if has_pending_changes {
                        ...
                        let btn_color = ACCENT_PRIMARY;
                        let btn_bg = if is_hovered {
                            lighten(btn_color, 0.1)
                        } else {
                            btn_color
                        };
        ...
        card_frame()
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        // Status badge
                        let (label, bg, fg) = if roblox_running {
                            ("Running", STATUS_WARNING.gamma_multiply(0.15), STATUS_WARNING)
                        } else {
                            ("Protected", STATUS_CONNECTED.gamma_multiply(0.1), STATUS_CONNECTED)
                        };
                        egui::Frame::NONE
                            .fill(bg)
                            .rounding(8.0)
                ...
                if roblox_running {
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ui.label(
                                egui::RichText::new("Applying while Roblox is running will restart Roblox.")
                                    .size(10.0)
        ...
        let response = egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ui.label(
                            egui::RichText::new(boost.name)
                                .size(12.0)
                                .color(TEXT_PRIMARY)
                                .strong(),
                        );
                        ui.label(
                            egui::RichText::new(boost.description)
                                .size(10.0)
                                .color(TEXT_MUTED),
                        );
                    });
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        self.animations
                            .animate_toggle(&toggle_id, is_enabled, anim_val);
                        ...
                        ui.painter()
                            .rect_filled(toggle_rect, toggle_height / 2.0, track_color);

                        // Knob
                        let knob_radius = (toggle_height - 6.0) / 2.0;
                        let knob_x = toggle_rect.left()
                            + toggle_height / 2.0
                            + current_anim * (toggle_width - toggle_height);
                        let knob_center = egui::pos2(knob_x, toggle_rect.center().y);
                        ui.painter()
                            .circle_filled(knob_center, knob_radius, TEXT_PRIMARY);
                ...
                ui.add_space(6.0);
                ui.horizontal(|ui| {
                    let impact_color = if is_enabled {
                        ACCENT_PRIMARY
                    } else {
                        TEXT_DIMMED
                    };
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ui.label(
                                egui::RichText::new(boost.impact)
                                    .size(9.0)
                                    .color(impact_color),
                            );
                        });
    ...
    /// Set boost value by ID (changes are applied when user presses Apply)
    fn set_boost_value(&mut self, id: &str, value: bool) {
        ...
        match id {
            ...
            "timer_resolution" => {
                self.state.config.system_optimization.timer_resolution_1ms = value
            }
            ...
            "network_throttling" => {
                self.state
                    .config
                    .network_settings
                    .disable_network_throttling = value
            }
    ...
    fn render_system_protection_card(&mut self, ui: &mut egui::Ui) {
        ...
        card_frame().show(ui, |ui| {
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new("System Protection")
                        .size(13.0)
                        .color(TEXT_PRIMARY)
                        .strong(),
                );
            ...
            ui.add_space(4.0);
            ui.label(
                egui::RichText::new("Create restore points before making changes")
                    .size(11.0)
                    .color(TEXT_MUTED),
            );
            ...
            ui.horizontal(|ui| {
                if ui
                    .add(
                        egui::Button::new(
                            egui::RichText::new("Create Restore Point")
                                .size(12.0)
                                .color(TEXT_PRIMARY),
                        )
                        .fill(ACCENT_PRIMARY)
                ...
                if ui
                    .add(
                        egui::Button::new(
                            egui::RichText::new("Open System Restore")
                                .size(12.0)
    ...
    pub(crate) fn render_toggle_row(
        &mut self,
        ui: &mut egui::Ui,
        label: &str,
        description: &str,
    ...
    pub(crate) fn render_animated_toggle_row(
        ...
        let response = ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.painter()
                    .rect_filled(toggle_rect, toggle_height / 2.0, bg_color);
