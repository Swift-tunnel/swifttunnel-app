...
use crate::network_analyzer::{
    SpeedTestProgress, StabilityTestProgress, run_speed_test, run_stability_test,
};
use std::sync::Arc;
...
impl BoosterApp {
    ...
    pub(crate) fn render_network_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label(
                egui::RichText::new("Network Analyzer")
                    .size(20.0)
                    .color(TEXT_PRIMARY)
                    .strong(),
            );
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.label(
                    egui::RichText::new("Test your connection")
                        .size(12.0)
                        .color(TEXT_MUTED),
                );
            });
    ...
    pub(crate) fn render_stability_section(&mut self, ui: &mut egui::Ui) {
        card_frame().show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label(
                    egui::RichText::new("Connection Stability")
                        .size(15.0)
                        .color(TEXT_PRIMARY)
                        .strong(),
                );
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if let Some(ref results) = self.network_analyzer_state.stability.results {
                        let (badge_color, badge_text) = match results.quality {
                            crate::network_analyzer::ConnectionQuality::Excellent => {
                                (STATUS_CONNECTED, "Excellent")
                            }
                            crate::network_analyzer::ConnectionQuality::Good => {
                                (LATENCY_GOOD, "Good")
                            }
                            crate::network_analyzer::ConnectionQuality::Fair => {
                                (STATUS_WARNING, "Fair")
                            }
                            crate::network_analyzer::ConnectionQuality::Poor => {
                                (LATENCY_POOR, "Poor")
                            }
                            crate::network_analyzer::ConnectionQuality::Bad => {
                                (STATUS_ERROR, "Bad")
                            }
                        };
                        egui::Frame::NONE
            ...
            if let Some(ref results) = self.network_analyzer_state.stability.results {
                ui.horizontal(|ui| {
                    ...
                    self.render_stat_box(
                        ui,
                        "Avg Ping",
                        &format!("{:.0}ms", results.avg_ping),
                        latency_color(results.avg_ping as u32),
                    );
                    ...
                    self.render_stat_box(
                        ui,
                        "Jitter",
                        &format!("{:.1}ms", results.jitter),
                        TEXT_SECONDARY,
                    );
                    ui.add_space(SPACING_SM);
                    // Packet Loss
                    let loss_color = if results.packet_loss < 1.0 {
                        STATUS_CONNECTED
                    } else if results.packet_loss < 5.0 {
                        STATUS_WARNING
                    } else {
                        STATUS_ERROR
                    };
                    ...
                    self.render_stat_box(
                        ui,
                        "Min/Max",
                        &format!("{}/{}ms", results.min_ping, results.max_ping),
                        TEXT_MUTED,
                    );
                });
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(format!(
                            "Testing... {:.0}%",
                            self.network_analyzer_state.stability.progress * 100.0
                        ))
                        .size(13.0)
        ...
        if samples.len() >= 2 {
            ...
            for (i, sample) in visible_samples.iter().enumerate() {
                if let Some(ms) = sample {
                    ...
                    let y = rect.max.y
                        - ((*ms as f32).min(max_ms) / max_ms) * rect.height() * 0.9
                        - 5.0;
    ...
    pub(crate) fn start_stability_test(&mut self) {
        ...
        std::thread::spawn(move || {
            let _ = rt.block_on(async { run_stability_test(30, tx).await });
        });
    }
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if let Some(ref results) = self.network_analyzer_state.speed.results {
                        ui.label(
                            egui::RichText::new(format!("via {}", results.server))
                                .size(11.0)
                                .color(TEXT_MUTED),
                        );
                    }
            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    self.render_speed_gauge(
                        ui,
                        "Download",
                        self.network_analyzer_state.speed.download_speed,
                        ACCENT_CYAN,
                        true,
                    );
                });
                ...
                ui.vertical(|ui| {
                    ...
                    self.render_speed_gauge(
                        ui,
                        "Upload",
                        self.network_analyzer_state.speed.upload_speed,
                        ACCENT_SECONDARY,
                        false,
                    );
                });
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        let progress_rect = ui
                            .allocate_exact_size(egui::vec2(120.0, 4.0), egui::Sense::hover())
                            .0;
                        let painter = ui.painter();
    ...
    pub(crate) fn render_speed_gauge(
        &self,
        ui: &mut egui::Ui,
        label: &str,
        speed: f32,
        ...
        let (response, painter) = ui.allocate_painter(
            egui::vec2(ui.available_width(), gauge_size),
            egui::Sense::hover(),
        );
        let rect = response.rect;
        ...
        for (ratio, label) in [
            ...
            let marker_pos = center
                + egui::vec2(
                    angle.cos() * (radius + 18.0),
                    -angle.sin() * (radius + 18.0),
                );
            painter.text(
                marker_pos,
                egui::Align2::CENTER_CENTER,
    ...
    pub(crate) fn start_speed_test(&mut self) {
        ...
        let rt = Arc::clone(&self.runtime);
        std::thread::spawn(move || {
            let _ = rt.block_on(async { run_speed_test(tx).await });
        });
    }
