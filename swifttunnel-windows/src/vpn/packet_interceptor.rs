...
impl PacketInterceptor {
    ...
    ///
    /// FIX: Now also checks destination IP - only tunnels traffic to game servers.
    ...
    fn should_route_to_vpn(
        ...
        let process_should_tunnel =
            process_tracker.should_tunnel(info.src_ip, info.src_port, info.protocol);

        // FIX: Also check if destination is a game server IP
        // Only tunnel to game servers, bypass auth/API traffic to avoid 403 errors
        let is_game_server = super::routes::is_roblox_game_server(info.dst_ip);
        let should_tunnel = process_should_tunnel && is_game_server;
        ...
        if log_decisions {
            if let Some(pid) = process_tracker.get_pid(info.src_ip, info.src_port, info.protocol) {
                let proc_name = process_tracker
                    ...
                    .map(|s| s.as_str())
                    .unwrap_or("unknown");
                let reason = if process_should_tunnel && !is_game_server {
                    "BYPASS (auth/API)"
                } else if should_tunnel {
                    "TUNNEL"
                } else {
                    "BYPASS"
                };
