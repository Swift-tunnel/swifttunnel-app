...
impl BoosterApp {
    ...
    pub(crate) fn render_game_preset_selector(&mut self, ui: &mut egui::Ui) {
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("Game Selection")
                            .size(14.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                ...
                let cols = if (available - card_spacing * 2.0) / 3.0 >= 140.0 {
                    3
                } else {
                    2
                };
                let card_width =
                    ((available - card_spacing * (cols as f32 - 1.0)) / cols as f32).floor();
                ...
                if self.selected_game_presets.is_empty() {
                    ...
                    ui.horizontal(|ui| {
                        ui.label(
                            egui::RichText::new(
                                "Select at least one game to enable split tunneling",
                            )
                            .size(11.0)
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(20.0);
                    ui.label(
                        egui::RichText::new("Sign In Required")
                            .size(20.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                    ui.add_space(8.0);
                    ui.label(
                        egui::RichText::new("Connect to gaming servers and reduce your ping")
                            .size(14.0)
                    ...
                    if ui
                        .add(
                            egui::Button::new(
                                egui::RichText::new("Go to Account Settings")
                                    .size(14.0)
                                    .color(TEXT_PRIMARY),
                            )
                            .fill(ACCENT_PRIMARY)
    ...
    pub(crate) fn render_connection_status(&mut self, ui: &mut egui::Ui) {
        ...
        let (status_text, _status_icon, status_color, detail_text, show_connected_info) =
            if instant_connecting {
                ...
                match &self.vpn_state {
                    ...
                    ConnectionState::ConfiguringSplitTunnel => (
                        ...
                        "",
                        STATUS_WARNING,
                        "Configuring split tunnel...".to_string(),
                        false,
                    ),
        ...
        let (uptime_str, split_tunnel_active, tunneled_processes) =
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if is_connected {
                        ...
                        let pulse = ((elapsed * std::f32::consts::PI / PULSE_ANIMATION_DURATION)
                            .sin()
                            + 1.0)
                            / 2.0;
                        ...
                        for i in 0..3 {
                            ...
                            ui.painter().circle_filled(
                                center,
                                radius,
                                STATUS_CONNECTED_GLOW.gamma_multiply(alpha),
                            );
                        }
                        ...
                        ui.painter().circle_filled(
                            center,
                            8.0,
                            STATUS_CONNECTED_GLOW.gamma_multiply(0.5),
                        );
                        ...
                        for i in 0..3 {
                            ...
                            ui.painter().circle_filled(
                                dot_pos,
                                4.0 - i as f32 * 0.5,
                                STATUS_WARNING.gamma_multiply(dot_alpha),
                            );
                        }

                        // Center dot
                        ui.painter()
                            .circle_filled(center, 6.0, STATUS_WARNING.gamma_multiply(0.3));
                    } else if is_error {
                        // Error state
                        ui.painter()
                            .circle_filled(center, 16.0, STATUS_ERROR.gamma_multiply(0.2));
                        ui.painter().circle_stroke(
                            center,
                            16.0,
                            egui::Stroke::new(2.0, STATUS_ERROR),
                        );
                        ui.painter().text(
                            center,
                        ...
                        ui.painter().circle_stroke(
                            center,
                            16.0,
                            egui::Stroke::new(1.5, STATUS_INACTIVE),
                        );
                    }
                ...
                if show_connected_info {
                    ...
                    ui.horizontal(|ui| {
                                ...
                                .show(ui, |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.vertical(|ui| {
                                            ui.spacing_mut().item_spacing.y = 1.0;
                                            ui.label(
                                                egui::RichText::new("Split Tunnel")
                                                    .size(10.0)
                                                    .color(TEXT_MUTED),
                                            );
                                            ui.label(
                                                egui::RichText::new(tunnel_text)
                                                    .size(11.0)
                                                    .color(tunnel_color),
                                            );
                                        });
                                ...
                                .show(ui, |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.vertical(|ui| {
                                            ui.spacing_mut().item_spacing.y = 1.0;
                                            ui.label(
                                                egui::RichText::new("Custom Relay")
                                                    .size(10.0)
                                                    .color(TEXT_MUTED),
                                            );
                        ...
                        if is_tester_badge && self.experimental_mode && self.auto_routing_enabled {
                            let (game_region_name, is_bypassed) = self
                                .vpn_connection
                                .try_lock()
                                .ok()
                                .and_then(|conn| {
                                    conn.auto_router().map(|r| {
                                        let region = r
                                            .current_game_region()
                                            .map(|rg| rg.display_name().to_string());
                            ...
                            let (badge_text, badge_color) =
                                if let Some(ref region) = game_region_name {
                                    if is_bypassed {
                                        (
                                            format!("Bypassed: {} (whitelisted)", region),
                                            STATUS_WARNING,
                                        )
                                    } else {
                                ...
                                .show(ui, |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.vertical(|ui| {
                                            ui.spacing_mut().item_spacing.y = 1.0;
                                            ui.label(
                                                egui::RichText::new("Auto Routing")
                                                    .size(10.0)
                                                    .color(TEXT_MUTED),
                                            );
                                            ui.label(
    ...
    pub(crate) fn render_connection_progress_steps(&self, ui: &mut egui::Ui) {
        ...
        let steps = [(1, "Config"), (2, "Adapter"), (3, "Tunnel"), (4, "Route")];
                    ...
                    ui.vertical_centered(|ui| {
                        ...
                        if is_current {
                            ...
                            ui.painter().circle_filled(
                                rect.center(),
                                glow_radius,
                                dot_color.gamma_multiply(0.3),
                            );
                        }
                        ui.painter().circle_filled(rect.center(), 4.0, dot_color);

                        // Step label
                        let label_color = if is_complete || is_current {
                            TEXT_PRIMARY
                        } else {
                            TEXT_MUTED
                        };
    ...
    pub(crate) fn update_throughput_history(&mut self) {
        ...
        let Some(stats) = &self.throughput_stats else {
            return;
        };
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label(
                            egui::RichText::new("Tunnel Traffic")
                                .size(10.0)
                                .color(TEXT_MUTED),
                        );
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ui.label(
                                egui::RichText::new(format!(
                                    "DN {}",
                                    format_bytes_per_sec(current_rx)
                                ))
                                .size(10.0)
                            ...
                            ui.label(
                                egui::RichText::new(format!(
                                    "UP {}",
                                    format_bytes_per_sec(current_tx)
                                ))
                                .size(10.0)
                                .color(ACCENT_CYAN),
                            );
                    ...
                    if num_points >= 2 {
                        ...
                        // Draw TX line (cyan)
                        let tx_points: Vec<egui::Pos2> = self
                            .throughput_history
                            .iter()
                            .enumerate()
                            .map(|(i, (_, tx, _))| {
                                ...
                                let y = graph_rect.bottom()
                                    - ((*tx / max_throughput) as f32 * (graph_height - 4.0));
                                egui::pos2(x, y.max(graph_rect.top() + 2.0))
                            })
                            .collect();

                        // Draw RX line (green)
                        let rx_points: Vec<egui::Pos2> = self
                            .throughput_history
                            .iter()
                            .enumerate()
                            .map(|(i, (_, _, rx))| {
                                let x = graph_rect.left() + x_step * i as f32;
                                let y = graph_rect.bottom()
                                    - ((*rx / max_throughput) as f32 * (graph_height - 4.0));
                                egui::pos2(x, y.max(graph_rect.top() + 2.0))
                            })
                            .collect();
    ...
    pub(crate) fn render_region_selector(&mut self, ui: &mut egui::Ui) {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if is_loading {
                    ...
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ui.label(
                                egui::RichText::new(format!("{} regions", regions.len()))
                                    .size(10.0)
                                    .color(TEXT_MUTED),
                            );
                        });
            ...
            if is_loading {
                ...
                egui::Frame::NONE
                    ...
                    .show(ui, |ui| {
                        ui.vertical_centered(|ui| {
                            ui.add_space(8.0);
                            ui.label(
                                egui::RichText::new("Failed to load servers")
                                    .size(15.0)
                                    .color(TEXT_PRIMARY)
                                    .strong(),
                            );
                            ui.add_space(6.0);
                            ui.label(egui::RichText::new(err).size(12.0).color(TEXT_SECONDARY));
                            ui.add_space(16.0);
                            if ui
                                .add(
                                    egui::Button::new(
                                        egui::RichText::new("Retry").size(13.0).color(TEXT_PRIMARY),
                                    )
                                    .fill(ACCENT_PRIMARY)
                                    .rounding(8.0)
                ...
                egui::Frame::NONE
                    ...
                    .show(ui, |ui| {
                        ui.vertical_centered(|ui| {
                            ui.label(
                                egui::RichText::new("No servers available")
                                    .size(14.0)
                                    .color(TEXT_MUTED),
                            );
                        });
        ...
        let region_cols = if (available_width - card_spacing) / 2.0 - 24.0 >= 180.0 {
            2
        } else {
            1
        };
        let card_width = ((available_width - card_spacing * (region_cols as f32 - 1.0).max(0.0))
            / region_cols as f32)
            .floor();
                    ...
                    if let Some(region) = region_iter.next() {
                        ...
                        let (bg, border_color, border_width) = if is_selected {
                            (
                                lerp_color(
                                    ACCENT_PRIMARY.gamma_multiply(0.12),
                                    ACCENT_PRIMARY.gamma_multiply(0.18),
                                    hover_val,
                                ),
                                ACCENT_PRIMARY,
                                1.5,
                                ...
                                ui.vertical(|ui| {
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        egui::Frame::NONE
                                            ...
                                            .show(ui, |ui| {
                                                ui.label(
                                                    egui::RichText::new(&region.country_code)
                                                        .size(10.0)
                                            ...
                                            |ui| {
                                                ...
                                                if let Some(ms) = latency {
                                                    ...
                                                    ui.painter().circle_filled(
                                                        dot_rect.center(),
                                                        3.0,
                                                        lat_color.gamma_multiply(dot_alpha),
                                                    );
                                                } else if is_finding {
                                                    let elapsed =
                                                        self.app_start_time.elapsed().as_secs_f32();
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.with_layout(
                                            ...
                                            |ui| {
                                                ...
                                                if gear_btn.clicked() {
                                                    if self.server_selection_popup.as_ref()
                                                        == Some(&region.id)
                                                    {
                                                        self.server_selection_popup = None;
                                                    } else {
                                                        self.server_selection_popup =
                        ...
                        self.animations
                            .animate_hover(&card_id, is_hovered, hover_val);
    ...
    pub(crate) fn render_server_selection_popup(&mut self, ui: &mut egui::Ui) {
        ...
        let (servers_in_region, region_name): (Vec<(String, Option<u32>)>, String) = {
            ...
            if let Some(region) = regions.iter().find(|r| r.id == popup_region_id) {
                ...
                let server_list: Vec<(String, Option<u32>)> = region
                    .servers
                    .iter()
                    .map(|server_id| {
                        // Check if this is the best server
                        let latency = if best_server_latency
                            .map(|(best_id, _)| best_id == server_id)
                            .unwrap_or(false)
                        {
            ...
            .show(ui.ctx(), |ui| {
                ...
                let auto_response = egui::Frame::NONE
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new("Auto (Best Ping)")
                                    .size(13.0)
                                    .color(if is_auto {
                                        TEXT_PRIMARY
                                    } else {
                ...
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("Or select specific server:")
                            .size(11.0)
                            .color(TEXT_MUTED),
                    );
                });
                ...
                for (server_id, latency) in &servers_in_region {
                    ...
                    let server_response = egui::Frame::NONE
                        .fill(if is_selected {
                            ACCENT_SECONDARY.gamma_multiply(0.12)
                        } else {
                        ...
                        .stroke(egui::Stroke::new(
                            ...
                            if is_selected {
                                ...
                                BORDER_SUBTLE.gamma_multiply(0.5)
                            },
                        ))
                        .rounding(8.0)
                        .inner_margin(egui::Margin::symmetric(12, 8))
                        .show(ui, |ui| {
                            ui.horizontal(|ui| {
                                ui.label(egui::RichText::new(&display_name).size(12.0).color(
                                    if is_selected {
                                        TEXT_PRIMARY
                                    } else {
                                        TEXT_SECONDARY
                        ...
                        .clicked()
                    {
                        // Force this server
                        self.forced_servers
                            .insert(popup_region_id.clone(), server_id.clone());
                        self.mark_dirty();
                        should_close = true;
                    }
    ...
    pub(crate) fn render_skeleton_region_cards(&self, ui: &mut egui::Ui) {
        let available_width = self.content_area_width.min(ui.available_width());
        let card_spacing = 10.0;
        let skeleton_cols = if (available_width - card_spacing) / 2.0 - 24.0 >= 180.0 {
            2
        } else {
            1
        };
        let card_width = ((available_width - card_spacing * (skeleton_cols as f32 - 1.0).max(0.0))
            / skeleton_cols as f32)
            .floor();
                    ...
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.vertical(|ui| {
                                ...
                                let title_rect = ui
                                    .allocate_exact_size(
                                        egui::vec2(inner_width * 0.6, 14.0),
                                        egui::Sense::hover(),
                                    )
                                    .0;
                                self.render_skeleton_rect(
                                ...
                                let desc_rect = ui
                                    .allocate_exact_size(
                                        egui::vec2(inner_width * 0.8, 10.0),
                                        egui::Sense::hover(),
                                    )
                                    .0;
                                self.render_skeleton_rect(
                                ...
                                let bar_rect = ui
                                    .allocate_exact_size(
                                        egui::vec2((inner_width - 4.0).max(50.0), 3.0),
                                        egui::Sense::hover(),
                                    )
                                    .0;
                                self.render_skeleton_rect(
    ...
    pub(crate) fn render_skeleton_rect(
        &self,
        ...
        let shimmer_x = rect.left() - shimmer_width
            + (rect.width() + shimmer_width * 2.0) * ease_in_out_sine(shimmer_offset % 1.0);
    ...
    pub(crate) fn render_latency_slider(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("Practice Mode")
                            .size(14.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        egui::Frame::NONE
                            .fill(if self.artificial_latency_ms == 0 {
                                BG_ELEVATED
                            } else {
                                STATUS_WARNING.gamma_multiply(0.15)
                            })
                            .rounding(4.0)
                ...
                if response.drag_stopped() && self.vpn_state.is_connected() {
                    if new_latency != self.last_applied_latency {
                        log::info!(
                            "Latency change queued: {}ms -> {}ms (applying in {}s)",
                            self.last_applied_latency,
                            new_latency,
                            DEBOUNCE_SECS
                        );
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ui.label(
                            egui::RichText::new(format!("{}ms", LATENCY_MAX_MS))
                                .size(10.0)
                                .color(TEXT_MUTED),
                        );
                    });
                ...
                ui.label(
                    egui::RichText::new("Add artificial latency for high-ping practice (5s delay)")
                        .size(11.0)
                        .color(TEXT_MUTED),
                );
            });
    ...
    fn render_tunnel_diagnostics(&self, ui: &mut egui::Ui) {
        ...
        let diagnostics = self
            ...
            .try_lock()
            .ok()
            .and_then(|v| v.get_split_tunnel_diagnostics());
        ...
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ui.label(
                        egui::RichText::new("Tunnel Status")
                            .size(13.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                ...
                if let Some((adapter_name, has_default_route, tunneled, bypassed)) = diagnostics {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if has_default_route {
                            egui::Frame::NONE
                                ...
                                .show(ui, |ui| {
                                    ui.label(
                                        egui::RichText::new("OK").size(9.0).color(STATUS_CONNECTED),
                                    );
                                });
                        } else {
                            egui::Frame::NONE
                                ...
                                .response
                                .on_hover_text(
                                    "Adapter may not have default route - check network settings",
                                );
                        }
                    });
                    ...
                    ui.horizontal(|ui| {
                        // Tunneled packets
                        ui.label(
                            egui::RichText::new("Tunneled:")
                                .size(11.0)
                                .color(TEXT_MUTED),
                        );
                        ui.add_space(4.0);
                        let tunneled_color = if tunneled > 0 {
                            STATUS_CONNECTED
                        } else {
                            TEXT_MUTED
                        };
                        ui.label(
                            egui::RichText::new(format_packet_count(tunneled))
                                .size(11.0)
                                .color(tunneled_color),
                        );
                        ...
                        ui.add_space(4.0);
                        ui.label(
                            egui::RichText::new(format_packet_count(bypassed))
                                .size(11.0)
                                .color(TEXT_SECONDARY),
                        );
                    });
    ...
    pub(crate) fn render_smart_selection_overlay(
        &mut self,
        ui: &mut egui::Ui,
        selection: super::SmartServerSelection,
    ) {
        let elapsed = selection.started_at.elapsed();
        let progress =
            (selection.current_index as f32 / selection.servers.len().max(1) as f32).min(1.0);
            ...
            .show(ui, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.painter().circle_stroke(
                        center,
                        36.0,
                        egui::Stroke::new(2.0, ACCENT_PRIMARY.gamma_multiply(0.2)),
                    );
                    ui.painter().circle_stroke(
                        center,
                        28.0,
                        egui::Stroke::new(1.5, ACCENT_PRIMARY.gamma_multiply(0.15)),
                    );
                    ui.painter().circle_stroke(
                        center,
                        20.0,
                        egui::Stroke::new(1.0, ACCENT_PRIMARY.gamma_multiply(0.1)),
                    );
                    ...
                    for (i, server_id) in selection.servers.iter().enumerate() {
                        let angle = (i as f32 / num_servers.max(1) as f32) * std::f32::consts::TAU
                        ...
                        let dot_size = if i == selection.current_index {
                            5.0
                        } else {
                            4.0
                        };
                    ...
                    // Title
                    ui.label(
                        egui::RichText::new("Finding Best Server")
                            .size(16.0)
                            .color(TEXT_PRIMARY)
                            .strong(),
                    );
                    ...
                    // Region name
                    ui.label(
                        egui::RichText::new(format!("Testing servers in {}", region_name))
                            .size(12.0)
                            .color(TEXT_SECONDARY),
                    );
                    ...
                    if fill_width > 0.0 {
                        ...
                        // Shimmer effect
                        let shimmer_x =
                            bar_rect.left() + (elapsed_secs * 50.0) % (bar_width + 30.0) - 15.0;
                        let shimmer_rect = egui::Rect::from_min_size(
                            egui::pos2(
                                shimmer_x.max(bar_rect.left()).min(fill_rect.right() - 10.0),
                                bar_rect.top(),
                            ),
                        ...
                        if shimmer_rect.left() < fill_rect.right() {
                            ui.painter().rect_filled(
                                shimmer_rect,
                                2.0,
                                ACCENT_PRIMARY.gamma_multiply(1.3),
                            );
                        }
                    }
                    ...
                    if let Some((best_id, best_ms)) = &selection.best_server {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.label(
                                egui::RichText::new("Best so far:")
                                    .size(10.0)
                                    .color(TEXT_MUTED),
                            );
                            ui.label(
                                egui::RichText::new(format!("{} ({}ms)", best_name, best_ms))
                                    .size(10.0)
                                    .color(STATUS_CONNECTED),
                            );
                        });
