...
- Auto Route, whitelisted regions, and forced-server changes apply to the live session on settings save; custom-relay sessions never run the auto-routing lookup task. Disabling Auto Route also cancels in-flight lookups/switches (held packets release on the current relay) and revokes an active whitelist bypass, as does un-whitelisting the current game region — but a bypass is never newly engaged outside a lookup boundary.
