//! Roblox Log Watcher (macOS)
//!
//! Monitors Roblox log files to detect game server IPs.
//! On macOS, Roblox logs are stored in ~/Library/Logs/Roblox/
...
/// Get Roblox logs directory path on macOS
///
/// macOS Roblox stores logs in ~/Library/Logs/Roblox/
fn get_roblox_logs_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join("Library").join("Logs").join("Roblox"))
}
