...
/// Get process name by PID using libproc (macOS)
///
/// Uses libproc::proc_pid::pidpath for fast process name lookup.
...
fn get_process_name_by_pid_fast(pid: u32) -> Option<String> {
    super::process_tracker::get_process_name(pid)
}

/// Look up PID for a specific connection via libproc (macOS)
///
/// This is an EXPENSIVE synchronous call - only use when cache misses.
/// Critical for guaranteeing first-packet tunneling when process watcher
/// has detected a process but the connection tables haven't been refreshed yet.
fn lookup_connection_pid(local_ip: Ipv4Addr, local_port: u16, protocol: Protocol) -> Option<u32> {
    super::process_tracker::lookup_pid_for_connection(local_ip, local_port, protocol)
}
...
impl ProcessSnapshot {
    ...
    /// This guarantees first-packet tunneling for kqueue-detected processes.
    ///
    /// STABILITY FIX (v1.0.8): Skip expensive libproc scans in V3 mode.
    /// The full process enumeration calls can be slow and cause latency spikes
    /// when combined with BPF packet capture. V3 mode relies on speculative
    ...
    fn is_tunnel_connection(&self, local_ip: Ipv4Addr, local_port: u16, protocol: Protocol) -> bool {
        ...
        // STABILITY FIX (v1.0.8): Skip blocking libproc scans in V3 mode
        // V3 relies on speculative tunneling (destination IP matching) instead
        // This prevents latency spikes from expensive process enumeration in hot path
        ...
        // Critical for kqueue-detected processes whose connections haven't
        ...
        if let Some(pid) = lookup_connection_pid(local_ip, local_port, protocol) {
            ...
            // This happens when:
            // 1. kqueue detected process and called register_process_immediate()
...
impl LockFreeProcessCache {
    ...
    /// Immediately register a process detected via kqueue
    ///
    /// Called when kqueue detects a watched process starting. This adds the
    ...
    ///
    /// With kqueue + this method:
    /// 1. kqueue notifies us instantly when process starts
    ...
    pub fn register_process_immediate(&self, pid: u32, name: String) {
        ...
        log::info!(
            "kqueue: Immediately registered process {} (PID: {}) for tunneling",
            name, pid
        );
    }
}
